...
public class ThPallina extends Thread{
    ...
    public void run() {
        while(true) {
            if (array[ptrScatola.getId()].ballTF) {                  //Se nella scatola è presente la pallina (ballTF=true)
    ...
    private void GestionePallinaVersoDestra() {
        if (array[ptrScatola.getId()].ballTF) {
        ...
        if ((array[ptrScatola.getId()].getIdTarget() % ptrDati.getNumColonne()) != 0) {
            if ((array[ptrScatola.getId()].isBallTF()) && (ptrDati.isSposta()) && (array[ptrScatola.getId()].getBall().getPosX() == (150 + (150 * ptrScatola.getId())) - (array[ptrScatola.getId()].getBall().getRaggio() / 2))) {     //se pallina è presente, se ha raggiunto una velocità sufficente e se tocca il bordo
    ...
    private void GestionePallinaVersoSinistra() {
        ...
        if ((ptrScatola.getId() % ptrDati.getNumColonne()) != 0) {
            if ((array[ptrScatola.getId()].isBallTF()) && (ptrDati.isSposta()) && (array[ptrScatola.getId()].getBall().getPosX() == (150 * ptrScatola.getId()) + (array[ptrScatola.getId()].getBall().getRaggio() / 2))) {             //se pallina è presente, se ha raggiunto una velocità sufficente e se tocca il bordo
